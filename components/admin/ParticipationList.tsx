...
export default function ParticipationList({
  ...
  const getEventTeams = (eventId: string) =>
            ...
            >
              <AccordionTrigger className="hover:no-underline px-6 py-5 group">
                <div className="flex items-center justify-between w-full pr-4">
                  ...
                  <Badge
                    variant="outline"
                    className="ml-4 bg-primary border-2 border-black text-black rounded-none font-bold"
                  >
                    {eventTeams.length}{" "}
                    {eventTeams.length === 1 ? "Team" : "Teams"}
                  </Badge>
                </div>
              </AccordionTrigger>
                ...
                {eventTeams.length === 0 ? (
                  ...
                  <div className="grid gap-4">
                    {eventTeams.map((team) => (
                      <Card
                        key={team.id}
                        className="bg-white border-2 border-black rounded-none shadow-sm"
                      >
